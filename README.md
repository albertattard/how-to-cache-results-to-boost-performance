...
1. We need to make sure that only one `Future` is added to the `cache` irrespective of the number of threads using it.  The following code fragment is responsible from this.
    ...
    a. The `putIfAbsent()` method will return the previous value attached with the given key.  If this is `null` it means that no value was attached with this key and that our `Future` instance was added to the map.
